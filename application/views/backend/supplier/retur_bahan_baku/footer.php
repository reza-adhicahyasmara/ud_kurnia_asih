...
<script type="text/javascript">
    var url_outlet = "<?php echo base_url('supplier/retur_bahan_baku'); ?>";
    var url = url_outlet ;
...
<script type="text/javascript">
    ...
    function load_data_detail_item_retur_bb(){
        var kode_retur_bb = "<?php echo $this->uri->segment(4); ?>";
        var status_retur_bb = $("#status_retur_bb").val();

        $.ajax({
            method : "POST",
            url : '<?php echo base_url('supplier/retur_bahan_baku/load_data_detail_item_retur_bb');?>',
            data : {
                kode_retur_bb:kode_retur_bb,
                status_retur_bb:status_retur_bb
            },
            beforeSend : function(){
                $('#content_item_retur_bb').html('<div style="text-align:center"><i class="bx bx-loader-alt bx-md bx-spin" style="margin-top: 30px; margin-bottom: 30px;" aria-hidden="true"></i></div>');
            },
...
<script>
    $(document).on('click', '#btn_kirim_retur', function(e) {
        ...
        Swal.fire({
            ...
            preConfirm: (response) => {       
                $.ajax({
                    url: '<?php echo base_url('supplier/retur_bahan_baku/update_retur_bb');?>',
                    method: 'POST',
                    data: {
                        kode_retur_bb:kode_retur_bb,
                        status_retur_bb:status_retur_bb,
